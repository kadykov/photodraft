...
def get_exif_data(image_path):
    ...
    try:
            ...
            for tag_id, value in exif_data_raw.items():
                ...
                if isinstance(value, bytes):
                    try:
                        ...
                        if (
                            tag_name == "XPKeywords"
                        ):  # Still attempt, but prioritize XMP dc:subject
            ...
            if xmp_info:
                ...
                def find_in_xmp(data, path_keys):
                    ...
                    for key_part in path_keys:
                        if isinstance(
                            current, list
                        ):  # If path leads to a list, take the first item
                            if current:
                                current = current[0]
                            else:
                                return None
                ...
                rdf_description = find_in_xmp(
                    xmp_info, ["xmpmeta", "RDF", "Description"]
                )
                if rdf_description:
                    # rdf_description might be a list of descriptions or a single one
                    descriptions = (
                        rdf_description
                        if isinstance(rdf_description, list)
                        else [rdf_description]
                    )
                    for desc_item in descriptions:
                        if isinstance(desc_item, dict):
                            ...
                            if (
                                subject_node
                                and isinstance(subject_node, dict)
                                and "Bag" in subject_node
                                and isinstance(subject_node["Bag"], dict)
                                and "li" in subject_node["Bag"]
                            ):
                                xmp_tags = subject_node["Bag"]["li"]
                                if isinstance(xmp_tags, list):
                                    xmp_data_dict["dc:subject"] = [
                                        str(tag).strip() for tag in xmp_tags if tag
                                    ]
                                    break  # Found tags
                            ...
                            elif "subject" in desc_item and isinstance(
                                desc_item["subject"], list
                            ):
                                xmp_data_dict["dc:subject"] = [
                                    str(tag).strip()
                                    for tag in desc_item["subject"]
                                    if tag
                                ]
                                break
                            elif "subject" in desc_item and isinstance(
                                desc_item["subject"], str
                            ):
                                xmp_data_dict["dc:subject"] = [
                                    desc_item["subject"].strip()
                                ]
                                break
        ...
        if "dc:subject" in xmp_data_dict:
            ...
            "XPKeywords" in exif_data
        ):  # Fallback to XPKeywords if dc:subject not found
    ...
    try:
        ...
        try:
            ...
            if xmp_info:
                ...
                rdf_description = find_in_xmp_debug(
                    xmp_info, ["xmpmeta", "RDF", "Description"]
                )
                if rdf_description:
                    descriptions = (
                    ...
                    for desc_item in descriptions:
                        if isinstance(desc_item, dict):
                            ...
                            if (
                                ...
                                print(
                                    f"    dc:subject (from Bag/li): {subject_node['Bag']['li']}"
                                )
                                break
                            elif "subject" in desc_item:  # Simpler structure
                                print(
                                    f"    dc:subject (direct): {desc_item['subject']}"
                                )
                                break
                    else:
...
def main(args):  # Modified to accept args
            ...
            try:
                ...
                if len(path_parts) >= 4:  # Expects YYYY/MM/DD/filename.jpg
                    try:
                        ...
                        print(
                            f"Warning: Could not parse date from path for {relative_path}. Skipping date parts."
                        )
                ...
                exif_data = get_exif_data(
                    image_path
                )  # This now returns combined EXIF and XMP

                date_taken_str = exif_data.get("DateTimeOriginal") or exif_data.get(
                    "DateTime"
                )
                ...
                image_data = {
                    ...
                    "focalLength": format_ifd_rational_value(
                        exif_data.get("FocalLength")
                    ),
                    "apertureValue": format_ifd_rational_value(
                        exif_data.get("FNumber")
                    ),
                    "isoSpeedRatings": exif_data.get("ISOSpeedRatings"),
                    "exposureTime": format_ifd_rational_value(
                        exif_data.get("ExposureTime")
                    ),
                }
    ...
    all_images_data.sort(
        key=lambda x: x.get("dateTaken") or "0000-00-00T00:00:00", reverse=True
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate a JSON manifest from image metadata."
    )
    parser.add_argument(
        "--debug-image",
        type=str,
